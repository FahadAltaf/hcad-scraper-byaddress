...
namespace hcad.scraper.byaddress
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                int pages = (apns.Count() + settings.entriesProcessedInSingleThread - 1) / settings.entriesProcessedInSingleThread;
                List<Task> taskList = new List<Task>();
                for (int i = 1; i <= pages; ++i)
                {
                    int index = i - 1;
                    Task task = Task.Factory.StartNew(() => ScrapeByAddress(apns.Skip(index * settings.entriesProcessedInSingleThread).Take(settings.entriesProcessedInSingleThread).ToList()));
                    taskList.Add(task);
                    if (i % settings.threads == 0 || i == pages)
                    {
        ...
        private static void ScrapeByAddress(List<string> addresses)
        {
            try
            {
                ...
                options.AddArguments((IEnumerable<string>)new List<string>()
            {
                ...
                using (IWebDriver driver = (!settings.showGUI) ? ((IWebDriver)new ChromeDriver(defaultService, options)) : ((IWebDriver)new ChromeDriver()))
                {
                    foreach (var address in addresses)
                    {
                        ...
                        DataModel model = new DataModel() { SearchedAddress = address };
                        bool scraped = false;
                        int tryCount = 1;
                        do
                        {
                            message = "";
                            Console.WriteLine("Searched Address: \"{0}\"", address);
                            try
                            {
                                var url = "https://public.hcad.org/records/QuickSearch.asp";

                                var stNumber = address.Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries);
                                ...
                                if (allDigits(stNumber[0]))
                                {
                                    ...
                                    var addressWithStNumber = address.Substring(address.IndexOf(' ') + 1);
